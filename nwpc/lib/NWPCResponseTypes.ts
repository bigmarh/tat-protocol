...
export type MessageHook = (message: any, context: NWPCContext) => Promise<boolean>;
...
export type NWPCHandler = (request: NWPCRequest, context: NWPCContext, res: NWPCResponseObject, next: () => Promise<void>) => Promise<NWPCResponse | void>;
...
export class NWPCResponseObject {
    ...
    async send(data: any, recipient?: string | string[]): Promise<NWPCResponse> {
        console.log('NWPCResponseObject: send', data, recipient);
        ...
        if (targetRecipient) {
            if (Array.isArray(targetRecipient)) {
                if (this.sender instanceof NWPCServer) {
                    ...
                    await Promise.all(targetRecipient.map(pubkey =>
    ...
    async error(code: number, message: string, recipient?: string | string[]): Promise<NWPCResponse> {
    ...
    async notFound(message?: string, recipient?: string | string[]): Promise<NWPCResponse> {
        return this.error(404, message || 'Not found', recipient);
    }

    async badRequest(message?: string, recipient?: string | string[]): Promise<NWPCResponse> {
        return this.error(400, message || 'Bad request', recipient);
    }

    async unauthorized(message?: string, recipient?: string | string[]): Promise<NWPCResponse> {
        return this.error(401, message || 'Unauthorized', recipient);
    }

    async internalError(message?: string, recipient?: string | string[]): Promise<NWPCResponse> {
