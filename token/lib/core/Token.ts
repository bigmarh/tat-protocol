...
import { KeyPair, TokenType } from '@tat-protocol/types';
...
export default class Token {
     ...
     */
    async create_token_hash(readerPubkey?: string, timeWindow?: number): Promise<string> {
    ...
    */
        ...
        switch (this.header.typ) {
            ...
            case TokenType.NON_FUNGIBLE:
                if (this.payload.tokenID === undefined) {
                    throw new Error('Non-fungible token must have a tokenID');
                }
                break;
            case TokenType.SEMI_FUNGIBLE:
                if (this.payload.amount === undefined || this.payload.tokenID === undefined) {
                    throw new Error('Semi-fungible token must have both amount and tokenID');
                }
                break;
            default:
     ...
     */
    isLocked(): boolean {
        return !!(this.payload.P2PKlock || this.payload.HTLC || this.payload.timeLock);
    }
