import { generateSecretKey, getPublicKey, nip44, verifyEvent, Event } from 'nostr-tools';
import { bytesToHex, hexToBytes } from '@noble/hashes/utils';
import NDK, { NDKEvent, NDKKind, NDKPrivateKeySigner, NDKTag } from '@nostr-dev-kit/ndk';
...
export async function postToFeed(ndk: NDK, message: string, fromKeys: KeyPair, tags: NDKTag[] = []) {
...
export async function updateProfile(ndk: NDK, fromKeys: KeyPair, profileData: ProfileData) {
    const signer = new NDKPrivateKeySigner(fromKeys.secretKey);
    const timestamp = Math.floor(Date.now() / 1000); // Use Unix timestamp in seconds
...
export async function getFollows(ndk: NDK, pubkey: string) {
    ...
    const latestEvent = Array.from(events).sort((a, b) => b.created_at - a.created_at)[0];
...
export async function Wrap(ndk: NDK, message: string, fromKeys: KeyPair, To: string) {
    ...
    giftwrap.content = nip44Encrypt(JSON.stringify(envelope), toUint8Array(postMan.secretKey), To);
...
export async function Unwrap(wrapped: string, localKeys: KeyPair, wrappedPubKey: string) {
    try {
        //Unwrap gift
        const unwrapped = JSON.parse(nip44Decrypt(wrapped, toUint8Array(localKeys.secretKey), wrappedPubKey));
        //Open Envelope
        const openEnv = JSON.parse(nip44Decrypt(unwrapped.content, toUint8Array(localKeys.secretKey), unwrapped.pubkey));
...
export function nip44Encrypt(message: string, senderPriv: Uint8Array, recPubKey: string) {
    const key = nip44.getConversationKey(senderPriv, recPubKey);
    return nip44.encrypt(message, key);
}

export function nip44Decrypt(ciphertext: string, recPriv: Uint8Array, senderPubKey: string) {
